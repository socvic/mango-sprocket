...
function randomString(length: number): string {
  ...
  return result.trim() || "note";
}

function randomOtherPrincipal(senderAddress: string, allAddresses: string[]): string {
  const candidates = allAddresses.filter((address) => address !== senderAddress);
  if (candidates.length === 0) {
    throw new Error("No non-self recipient candidates available");
  }
  return candidates[randomInt(0, candidates.length - 1)] as string;
}

interface Interaction {
  contract: string;
  func: string;
  argsGen: (senderAddress: string, allAddresses: string[]) => ClarityValue[];
}
...
const CONTRACTS: { [key: string]: Omit<Interaction, "contract">[] } = {
  "daily-streaks": [
    ...
    {
      func: "check-in-with-note",
      argsGen: () => [someCV(stringUtf8CV(randomString(24)))],
    },
    {
      func: "check-in-with-note",
      argsGen: () => [noneCV()],
    ...
    {
      func: "add-friend",
      argsGen: (senderAddress, allAddresses) => [
        standardPrincipalCV(randomOtherPrincipal(senderAddress, allAddresses)),
      ],
    },
    {
      func: "create-group",
      argsGen: () => [stringUtf8CV(`Group ${randomString(10)}`), uintCV(randomInt(2, 20))],
    },
  ],
};
...
async function main() {
  ...
  console.log("Interacting with Daily Streaks (Burst Mode)...");
  ...
  const derivedWallets = walletIndexes.map((walletIndex) => {
    const account = wallet.accounts[walletIndex];
    const address = privateKeyToAddress(account.stxPrivateKey, "mainnet");
    return { walletIndex, account, address };
  });
  const allAddresses = derivedWallets.map((item) => item.address);

  const contractNames = Object.keys(CONTRACTS);

  const selectedInteractions = derivedWallets.map(({ walletIndex, account, address }) => {
    const randomContractName = contractNames[randomInt(0, contractNames.length - 1)] as string;
    const potentialFunctions = CONTRACTS[randomContractName] as Array<Omit<Interaction, "contract">>;
    const randomFuncDef = potentialFunctions[randomInt(0, potentialFunctions.length - 1)] as Omit<
      Interaction,
      "contract"
    >;

    const functionArgs = randomFuncDef.argsGen(address, allAddresses);
